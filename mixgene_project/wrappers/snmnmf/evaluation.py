...
class EnrichmentInGeneSets(SNMNMNMFEvaluation):
    ...
    def computeFisherExactTest(self, all_genes, cm_genes, bpGO, enrichment_threshold=0.05):
        ...
        for term_name in bpGO.keys():
            ...
            b = len(list(
                cm_genes.intersection(all_genes.difference(goterm))))  # contains     co-module and not go-term genes
            c = len(list(
                all_genes.difference(cm_genes).intersection(goterm)))  # contains not co-module and     go-term genes
            d = len(list(all_genes.difference(cm_genes).intersection(
                all_genes.difference(goterm))))  # contains not co-module and not go-term genes
            #
            x = np.array([[a, b], [c, d]])
    ...
    def computeEnrichmentRatio(self, cm_set, geneSets, T, enrichment_threshold=0.05, N=100):
        ...
        for n in range(1, N, 1):
            ...
            random_count, tmp2 = self.computeFisherExactTest(all_genes, set(random_cm_genes), geneSets,
