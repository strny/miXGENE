...
urlpatterns = patterns('',
    ...
    url(r'^workflows/ro/(?P<exp_id>\d+)', 'webapp.views.exp_ro', name='exp_ro'),

    url(r'^workflows/(?P<exp_id>\d+)/duplicate/?$',
        'webapp.views.duplicate_experiment', name="duplicate_experiment"),

    url(r'^workflows/(?P<exp_id>\d+)/change_name/?$',
        'webapp.views.exp_change_name', name="exp_change_name"),

    url(r'^workflows/(?P<exp_id>\d+)/sub/(?P<sub>[\w|\d|_]+)?$',
        'webapp.views.exp_sub_resource', name="experiment_sub_resource"),

    url(r'^workflows/(?P<exp_id>\d+)/blocks/?$', 'webapp.views.blocks_resource', name="blocks_resource"),
    url(r'^workflows/(?P<exp_id>\d+)/log/?$', 'webapp.views.exp_log', name="exp_log"),
    ...
    url(r'^workflows/(?P<exp_id>\d+)/blocks/(?P<block_uuid>[\w|\d]+)/invoke/(?P<method>\w+)$',
        'webapp.views.block_method', name="block_method"),
    url(r'^workflows/(?P<exp_id>\d+)/blocks/(?P<block_uuid>[\w|\d]+)/(?P<field>\w+)/(?P<format>[\w|\d|_]+)$',
    ...
    url(r'^workflows$', 'webapp.views.workflows', name='workflows'),
